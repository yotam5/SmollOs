...
struct command_argument* smollos_parse_command(const char* command, int max)
{
    ...
    root_command = smollos_malloc(sizeof(struct command_argument));
    if (!root_command)
    {
        goto out;
    }
    ...
    while(token != 0)
    {
        struct command_argument* new_command = smollos_malloc(sizeof(struct command_argument));
        if (!new_command)
        {
            break;
        }
...
int smollos_system_run(const char* command)
{
    ...
    struct command_argument* root_command_argument = smollos_parse_command(buf, sizeof(buf));
