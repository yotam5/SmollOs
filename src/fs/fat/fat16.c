...
#define SmollOs_FAT16_SIGNATURE 0x29
#define SmollOs_FAT16_FAT_ENTRY_SIZE 0x02
#define SmollOs_FAT16_BAD_SECTOR 0xFF7
#define SmollOs_FAT16_UNUSED 0x00

typedef unsigned int FAT_ITEM_TYPE;
#define FAT_ITEM_TYPE_DIRECTORY 0
#define FAT_ITEM_TYPE_FILE 1

// fat directory entry attribues bitmask
...
struct fat_private {
  ...
  struct disk_stream *fat_read_stream;     // stream the file alloc table
...
int fat16_stat(struct disk* disk, void* _private, struct file_stat* stat);
int fat16_close(void* _private);
int fat16_write(struct disk* disk, void* descriptor, uint32_t size, uint32_t nmemb,char* in);
...
int fat16_get_total_items_for_directory(struct disk *disk,
  ...
  if (diskstreamer_seek(stream, directory_start_pos) != SmollOs_ALL_OK) {
    res = -EIO;
    goto out;
  }
  while (1) {
    if (diskstreamer_read(stream, &item, sizeof(item)) != SmollOs_ALL_OK) {
      res = -EIO;
      goto out;
    }
...
int fat16_get_root_directory(struct disk *disk, struct fat_private *fat_private,
  ...
  if (diskstreamer_read(stream, dir, root_dir_size) != SmollOs_ALL_OK) {
    res = -ENOMEM;
    goto out;
  }
  directory->item = dir;
...
int fat16_resolve(struct disk *disk) {
  ...
  if (diskstreamer_read(stream, &fat_private->header,
                        sizeof(fat_private->header)) != SmollOs_ALL_OK) {
    res = -EIO;
    goto out;
  }
  ...
  if (fat16_get_root_directory(
          disk, fat_private, &fat_private->root_directory) != SmollOs_ALL_OK) {
    res = -EIO;
    goto out;
  }
...
static int fat16_get_fat_entry(struct disk* disk, int cluster)
{
    ...
    res = diskstreamer_seek(stream, fat_table_position * (cluster * SmollOs_FAT16_FAT_ENTRY_SIZE));
    if(res < 0)
    {
        goto out;
    }
...
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster, int offset)
{
    int res = 0;
    struct fat_private* private = disk->fs_private;
    int size_of_cluster_byets = private->header.primary_header.sectors_per_cluster*disk->sector_size;
    int cluster_to_use = starting_cluster;
    int cluster_ahead = offset/size_of_cluster_byets;
    for(int i = 0; i < cluster_ahead;i++)
    {
        ...
        if(entry == SmollOs_FAT16_BAD_SECTOR)
        {
            res = -EIO;
            goto out;
        }
...
{
    //print("write internal from stram\n");
    int res = 0;
    struct fat_private* private = disk->fs_private;
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk,struct fat_directory* directory,const char* name)
{
    ...
    char tmp_filename[SmollOs_MAX_PATH];
...
int fat16_write(struct disk* disk, void* descriptor, uint32_t size, uint32_t nmemb,char* in)
{
    ...
    {
        res = fat16_write_internal(disk, fat16_get_first_cluster(item), offset, size, in);
        if(ISERR(res))
        {
            goto out;
        }
...
{
    int res = 0;
    struct fat_file_descriptor* descriptor = (struct fat_file_descriptor*)_private;
...
int fat16_close(void* _private)
{
    fat16_free_file_descriptor((struct fat_file_descriptor*)_private);
    return 0;
}